...
public class ShopApiController implements ShopApi {
    ...
    public ResponseEntity<OrderProcessResponse> getListOrderInQueue(@ApiParam(value = "clientId" ) @RequestHeader(value="clientId", required=false) String clientId, @ApiParam(value = "jwt token contain user infomation and user role" ) @RequestHeader(value="bearer", required=false) String bearer) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<OrderProcessResponse>(objectMapper.readValue("{\"empty\": false}", OrderProcessResponse.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<OrderProcessResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<OrderProcessResponse>(HttpStatus.NOT_IMPLEMENTED);
    }
